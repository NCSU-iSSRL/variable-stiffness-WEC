...
%regulator_set_pressure_PSI_in = [1 3 5 7 10];
%regulator_set_pressure_PSI_in = 10:5:50;
pto_stiffness_in = 200;
results = [];
simData = {};
    ...
    for pto_stiffness = pto_stiffness_in
        ...
        for wavePeriod = wavePeriods
            ...
            try
                ...
                timeVec = logsout{1}.Values.Time;
                avgPower = sum(output.ptos.powerInternalMechanics(timeVec>power_avg_start_time,3)) / (timeVec(end) - power_avg_start_time);
                results(end,:) = [waveHeight, wavePeriod, pto_stiffness, avgPower];
...
filename = sprintf('%s\\P3_sim_sweep_springDamper_pto_resultsOnly_%s.mat', dataSaveFolder, runTimeStamp);
...
switch getenv('COMPUTERNAME')
    case {"VERMILLIONLAB13", "MAE-DT-001"}
        dataSaveFolder = 'D:\research_data\WEC_sim_data\plunger3_buoy';
    otherwise
...
if saveLargeData
    filename = sprintf('%s\\P3_sim_sweep_springDamper_pto_full_%s.mat', dataSaveFolder, runTimeStamp);

...
job1 = createJob(c, 'AutoAddClientPath', true);

%%% runs sims using cluster
for gearRatioQ = 1:length(gear_ratio_in)
    for waveHeightQ = 1:2%length(waveHeights)
        for wavePeriodQ = 1:2%length(wavePeriods)
            ...
            if waveHeight <= 0.1023*wavePeriod.^2 + 0.0318*wavePeriod + 0.0501 + 0.25 % restrict sim domain to fall within typical wave conditions at CDIP buoys
                createTask(job1, @func_run_cluster_wecSim, 2, {waveHeight, wavePeriod, turbine_gear_ratio});
            end
        end
    end
end
